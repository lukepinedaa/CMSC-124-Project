    ...
    for line in lines:
        ...
        for i in range (0, len(words)):
            ...
            #for flagged words with not unique first word as keyword
                ...
                alreadyComplete = True #flag if current lexeme is complete as a keyword
                skip = 0
                for possibleNextWords in flaggedWords2[words[i]]:
                    for nextWord in possibleNextWords:
                        ...
                        if (i+skip+1) < len(words) and words[i+skip+1] == nextWord:
                            skip += 1
                            lexeme = lexeme + " " + nextWord
                        #not the expected next word of flagged word, reset lexeme to current word and reset skip's value to 0
                        else:
                            alreadyComplete = False
                            lexeme = words[i]
                            skip = 0
                    #if alreadyComplete remained True, all expected next word was found and concatenated
            ...
            for regex in regexPatterns:
                #if lexeme matches current regex, append lexeme and its type (in a dictionary) to currentToken, and append currentToken to currentLine
            ...
            if withMatch == False:
                return([False, lineCounter, "unknown token at: " + lexeme])
